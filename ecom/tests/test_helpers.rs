...
use std::error::Error;
use ecom::import_model::ImportOrder;
use ecom::order_storage::OrderStorage;
...
impl TestEcomOrderStorage {
    pub async fn new() -> Result<Self, Box<dyn Error + Send + Sync>> {
        ...
        let storage = OrderStorage::new(&database_url).await?;
    ...
    pub async fn save_test_transaction(&self, order: &ImportOrder) -> Result<i64, Box<dyn Error + Send + Sync>> {
        ...
        self.storage.save(order).await
    }
    
    /// Get transaction using the underlying storage
    pub async fn get_transaction(&self, id: i64) -> Result<EcomOrder, Box<dyn Error + Send + Sync>> {
