...
use crate::{
    model::{
        Processible, ProcessibleSerde, Transaction,
    },
...
pub use inputs::{FILTER_OPS_BOOL, FILTER_OPS_FLOAT, FILTER_OPS_INT, FILTER_OPS_STRING, build_typed_operator_inputs, operator_input_name_for};
...
fn to_transaction<'a>(parent_value: &'a FieldValue<'a>) -> &'a Transaction {
    parent_value.try_downcast_ref::<Transaction>()
...
fn transaction_object<P: ProcessibleSerde>(payload_type_name: &str) -> Object {
    ...
    .description("The transaction object, that contains customer provided data and all calculated data.")
    .field(
    ...
    .field(
        Field::new("payload_number", TypeRef::named_nn(TypeRef::STRING), |ctx| {
            FieldFuture::new(async move {
                let tx = to_transaction(ctx.parent_value);
    ...
    .field(
        Field::new("schema_version_major", TypeRef::named_nn(TypeRef::INT), |ctx| {
            FieldFuture::new(async move {
                let tx = to_transaction(ctx.parent_value);
    ...
    .field(
        Field::new("schema_version_minor", TypeRef::named_nn(TypeRef::INT), |ctx| {
            FieldFuture::new(async move {
                let tx = to_transaction(ctx.parent_value);
    ...
    .field(
        Field::new("last_scoring_date", TypeRef::named(TypeRef::STRING), |ctx| {
            FieldFuture::new(async move {
                let tx = to_transaction(ctx.parent_value);
    ...
    .field(
        Field::new("processing_complete", TypeRef::named_nn(TypeRef::BOOLEAN), |ctx| {
            FieldFuture::new(async move {
                let tx = to_transaction(ctx.parent_value);
...
pub fn schema<P: Processible + ProcessibleSerde + 'static>(common_storage: Arc<dyn CommonStorage>) -> Result<Schema, SchemaError> {
    ...
    for (column, scalar) in column_types.iter() {
        ...
        filters_input_object = filters_input_object
            .field(InputValue::new(column, TypeRef::named(ops_type_name)));
    }
