use crate::{
    model::*,
    queue::{ProdQueue, RecalcQueue, QueueService},
    scorers::Scorer,
...
impl<P: Processible + ProcessibleSerde, S: Scorer> Processor<P, S>
{
    ...
    pub async fn process(&self, transaction_id: ModelId) -> Result<(), Box<dyn Error + Send + Sync>> {
            ...
            .expect("Processing: Failed to save matching fields during processing");
        {
            ...
            h.record(Instant::now().duration_since(matching_extract_start).as_secs_f64());
        }
        ...
        for ct in &connected_transactions {
            if ct.transaction_id != transaction_id {
                if let Err(e) = self.recalc_queue.enqueue(ct.transaction_id).await {
                    error!("Failed to enqueue {} for recalculation: {}", ct.transaction_id, e);
                } else {
                    added_to_recalc += 1;
                }
            }
        }
        info!("Transactions added to the recalculation queue: {}", added_to_recalc);
        ...
        {
            let h = histogram!("frida_processing_stage_seconds", "stage" => "features_simple_extract");
        ...
        {
            let h = histogram!("frida_processing_stage_seconds", "stage" => "features_graph_extract");
            h.record(graph_extract_start.elapsed().as_secs_f64());
        }
        debug!("Processing: Simple features: {}", serde_json::to_string(&simple_features).unwrap_or_else(|_| "<serialize_error>".to_string()));
        debug!("Processing: Graph features: {}", serde_json::to_string(&graph_features).unwrap_or_else(|_| "<serialize_error>".to_string()));

        debug!("Processing: Saving features for transaction {:?}", &transaction_id);
    ...
    pub async fn recalculate(&self, transaction_id: ModelId) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        debug!("Recalculation: Graph features: {}", serde_json::to_string(&features).unwrap_or_else(|_| "<serialize_error>".to_string()));
        ...
        let score_start = Instant::now();
        self.score_and_save_results(transaction_id, all_features).await?;
        {
    ...
    async fn score_and_save_results(
        ...
        let activation = self.storage.get_activation_by_channel_id(self.scorer.channel_id()).await
            .map_err(|e| {
                error!("Failed to fetch activation for channel {}: {}", self.scorer.channel_id(), e);
                e
            })?;
        if let Err(e) = self.scorer.score_and_save_result(transaction_id, activation.id, features).await {
            error!("Failed to score and save result for transaction {}: {}", transaction_id, e);
            return Err(e);
        }
